...
var seasonsKillNodes = map[string]*Node{
	...
	// enemies vulnerable to scent seeds are always vulnerable to sword and
	// fool's ore (and punches?).
	...
	"kill moblin (gap)": Or("sword", "scent seeds", "slingshot kill normal",
		...
		And("hard", Or("punch enemy", "ember seeds"))),
	...
	"flip kill spiked beetle": And("flip spiked beetle", Or("sword", "fool's ore",
		"satchel kill normal", "slingshot kill normal")),
	...
	"finish manhandla": Or("sword", "any slingshot", "fool's ore",
		"expert's ring"),
